...
def main(page: Page):
    ...
    def close_and_change(e):
        datahelp.replaceData(
            yearText.value, monthText.value, businessText.value, IncomeText.value
        )
        dlg2.open = False
    ...
    def add_click(e):
        while True:
            if not datahelp.checkNoExist(
                yearText.value, monthText.value, businessText.value
            ):
                dlg2.open = True
                page.dialog = dlg2
            else:
    ...
    dlg2 = ft.AlertDialog(
        modal=True,
        title=ft.Text("We already have recored for those business and data!"),
        content=ft.Text("if you want to replace?"),
        actions=[
            ft.TextButton("Yes", on_click=close_and_change),
            ft.TextButton("No, Thank", on_click=close_dlg2),
        ],
    )

    dlg3 = ft.AlertDialog(
        modal=True,
        title=ft.Text("Record inserted!"),
        actions=[ft.TextButton("Ok", on_click=close_dlg3)],
    )
    ...
    IncomeText = TextField(
        value="", color=ft.colors.BLUE, text_size=40, on_submit=add_click
    )
