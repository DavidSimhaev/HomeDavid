...
class Turtle:
    ...
    def leftvec(self):
        return (-1*self.__vectors[self.__vector][1], # [1] - y = вертикаль
                self.__vectors[self.__vector][0]) # [0] - x = горизонталь
    
    def leftcell(self, x, y):
        ppoint = self.pointsum((x, y), self.leftvec()) # Считает два масива, возращает КОНЕЧНЫЙ РЕЗУЛЬТАТ
        return self.board.freecell(ppoint[0], ppoint[1]) # 
    
    def turnvector(self):
        if self.__vector == 3: # Если индекс вектора ровняется трем то вектор сбрасывается и начинает по новой
            self.__vector = 0
        else:
            self.__vector = self.__vector + 1
            .3
    def insideboard(self, x, y):
            return x <= self.board.n and y <= self.board.n and x >= 1 and y >= 1 # Определение рамок/стенок
        
    def validate(self, x, y):
        if self.insideboard(x,y): # Если массив не выходит за рамки стенок
            if self.board.freecell(x, y): # Если ровно нулю , то тру 
                self.board.board = x, y, self.__value
                self.__point = (x, y)
                if not self.leftcell(x, y):
                    return True
                self.turnvector() # Запускают проверку
                return True
        return False
