...
class Files:
    ...
    upload_bool_ = False
    def __init__(self, new_frame, FRAME_RECORDING, switch_value):
    ...
    def list_files_from_pr(self, New_Frame_Files, FRAME_RECORDING ):
        ...
        if self.FILES_LIST != []:
            ...
            def new_file():
                ...
                if name == "":
                    try:
                        ...
                        label= tk.Label(New_Frame_Files, text = "Please upload files or select a project", font = "Arial 10", bg= bg_val, fg=fg_val)
                        label.pack()
                    f()
                    return  
            ...
            def f():
                ...
                frame2 = tk.Frame(New_Frame_Files, bd=3, relief=tk.FLAT, bg= bg_val)
                frame2.grid(row=3, column=0, sticky=tk.NW)   
                canvas = tk.Canvas(frame2, bg= bg_val)
                canvas.grid(row=0, column=0)
                # Create a vertical scrollbar linked to the canvas.
                vsbar = tk.Scrollbar(frame2 , orient=tk.VERTICAL, command=canvas.yview, bg= bg_val)
                vsbar.grid(row=0, column=1, sticky=tk.NS)
                canvas.configure(yscrollcommand=vsbar.set)
                widget_frame = tk.Frame(canvas, bg= bg_val)
                for file in self.FILES_LIST:
                    if file == " ":
                        continue
                    Label_name = tk.Label(widget_frame, textvariable= file , text = file[-10::], cursor="hand2", bg= bg_val, fg=fg_val)
                    Label_name.grid(row= b ,column=0, padx=(1, 220))
                    
                    
                    Icon_shove = tk.Label(widget_frame ,image=self.img_shove, cursor="hand2",bg= bg_val)
