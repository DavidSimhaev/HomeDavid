...
from .models import Color,Category,Camera, lens, lightings, tripods, Binoculars, IMG_FILES, IMG_FILES_LENS, IMG_FILES_TRIPODS, IMG_FILES_LIGHTS, IMG_FILES_BINOCULARS, account_data
...
def Lens_filtred(request):
    ...
    try:
        ...
        color = None   
    product_1 = Camera.objects.filter(category= category)
    product_2 = Camera.objects.filter(firm= category_type)
    product_5 = Camera.objects.filter(color= color)
    product_6 = Camera.objects.filter(lens= ID_LENS)
    product_7 = Camera.objects.filter(hand= ID_HAND)
    list_product = [product_1,product_2,product_5, product_6, product_7]
    result = []
    len_objest = 4
    last_step = True
    first = True
